...
├── setup-self-hosted-runners.yml  # Setup self-hosted runners on Oracle
├── cleanup-self-hosted-runners.yml # Cleanup self-hosted runners
...
## 🏃 Self-Hosted Runners on Oracle Compute

This repository includes workflows for setting up and managing self-hosted GitHub Actions runners on the Oracle Compute instance. These runners provide:

- **Faster deployments** - No runner spin-up time
- **Direct access** - No SSH tunneling needed
- **Consistent environment** - Matches production environment
- **Cost savings** - Uses existing Oracle Free Tier compute

### 7. Setup Self-Hosted Runners: `setup-self-hosted-runners.yml`
...
| Input | Type | Required | Default | Description |
|-------|------|----------|---------|-------------|
| `runner-labels` | string | ❌ | `self-hosted,oracle,linux,kanban,cline` | Labels for runners |
| `runner-count` | choice | ❌ | `1` | Number of runners (1-3) |
| `runner-group` | string | ❌ | `default` | Runner group name |

**Secrets** (required):
- `ORACLE_HOST`, `ORACLE_USER`, `ORACLE_SSH_KEY_B64`, `DEPLOY_PATH`
- `GH_PAT` - GitHub PAT with `admin:repo_hook` and `repo` scopes

**Job**: `setup-runners` (runs on `ubuntu-latest`, timeout: 30 min)

**Steps**:
1. **Setup SSH Key**: Decode base64 key, configure known_hosts
2. **Test SSH Connection**: Verify connectivity
3. **Get Registration Token**: Call GitHub API to get runner registration token
4. **Create Setup Script**: Generate script to install runners on Oracle
5. **Execute on Oracle**: Run setup script via SSH

**Setup Script Actions** (per runner):
- Create runner directory: `$DEPLOY_PATH/github-runners/runner-{N}`
- Download GitHub Actions runner (v2.319.1)
- Configure runner: `./config.sh --url https://github.com/{repo} --token {token} --labels {labels} --name oracle-runner-{N} --unattended`
- Install as systemd service: `./svc.sh install`
- Start service: `./svc.sh start`

**Outputs**: Runner names created (e.g., `oracle-runner-1,oracle-runner-2`)

---

### 8. Cleanup Self-Hosted Runners: `cleanup-self-hosted-runners.yml`
...
|-------|------|----------|---------|-------------|
| `environment` | choice | ✅ | `production` | Deployment environment |
| `runner-names` | string | ❌ | - | Specific runner names (comma-separated) |
| `force` | boolean | ❌ | `false` | Force removal without confirmation |
...
**Jobs**:
1. **Get Runners from GitHub**: Query GitHub API for runners with `oracle-runner` prefix
2. **Remove from GitHub**: DELETE API calls to unregister runners
3. **Cleanup on Oracle**: SSH to Oracle, stop/uninstall services, remove directories

---

### 9. Deploy Using Self-Hosted Runners: `deploy-using-self-hosted.yml`
...
|-------|------|----------|---------|-------------|
| `environment` | choice | ✅ | `production` | Deployment environment |
| `use-self-hosted` | boolean | ❌ | `true` | Use self-hosted runners |

**Jobs** (all use `workflow_call` with conditional `runs-on`):
- `validate-yaml` → calls `validate-yaml.yml`
- `build-package` → calls `build-package.yml` (needs: validate-yaml)
- `auth-setup` → calls `auth-setup.yml` (needs: validate-yaml)
- `deploy-infrastructure` → calls `deploy-infrastructure.yml` (needs: auth-setup)
- `deploy-to-oci` → calls `deploy-to-oci.yml` (needs: build-package, auth-setup, deploy-infrastructure)
- `health-check` → calls `health-check.yml` (needs: deploy-to-oci)

**Conditional Runner Selection**:
```yaml
runs-on: ${{ github.event.inputs.use-self-hosted && '[self-hosted, oracle, linux]' || 'ubuntu-latest' }}
```

When `use-self-hosted=true`, jobs run on runners with labels `self-hosted`, `oracle`, and `linux`.
