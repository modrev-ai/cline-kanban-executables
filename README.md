...
## Self-Hosted GitHub Actions Runners on Oracle

This repository includes workflows for setting up and managing self-hosted GitHub Actions runners on the Oracle Compute instance.

### Setup Self-Hosted Runners

**Workflow:** `.github/workflows/setup-self-hosted-runners.yml`

**Trigger:**
```bash
gh workflow run setup-self-hosted-runners.yml --repo modrev-ai/cline-kanban-executables
```

**Inputs:**
- `runner-labels`: Labels for runners (default: `self-hosted,oracle,linux,kanban,cline`)
- `runner-count`: Number of runners (1-3, default: 1)
- `runner-group`: Runner group name (default: `default`)

**What it does:**
1. Downloads and installs GitHub Actions runner on Oracle instance
2. Configures runners with specified labels
3. Installs runners as systemd services (auto-start on boot)
4. Registers runners with GitHub

### Cleanup Self-Hosted Runners

**Workflow:** `.github/workflows/cleanup-self-hosted-runners.yml`

**Trigger:**
```bash
gh workflow run cleanup-self-hosted-runners.yml --repo modrev-ai/cline-kanban-executables
```

**Inputs:**
- `runner-names`: Specific runner names to remove (comma-separated, leave empty for all oracle runners)
- `force`: Force removal without confirmation

### Deploy Using Self-Hosted Runners

**Workflow:** `.github/workflows/deploy-using-self-hosted.yml`

**Trigger:**
```bash
gh workflow run deploy-using-self-hosted.yml --repo modrev-ai/cline-kanban-executables
```

**Inputs:**
- `use-self-hosted`: Use self-hosted runners instead of GitHub-hosted (default: true)

This workflow runs the full deployment pipeline (infrastructure, deploy, health-check) on the self-hosted runners, which provides:
- Faster deployments (no runner spin-up time)
- Direct access to Oracle instance (no SSH tunneling needed)
